...
export default function Home() {
  ...
  const [selectedCrop, setSelectedCrop] = useState(null); // Selected crop filter
      ...
      {!showSearch && !selectedCategory && (
        <section className="container mx-auto px-4 py-12 bg-gradient-to-b from-white to-gray-50">
          ...
          <div className="space-y-8">
            {[
              { name: 'नगदी पिके', crops: ['ऊस', 'कापूस', 'कॉफी', 'तंबाखू', 'केसरी', 'अळशी'], icon: '🌾', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
              { name: 'भाज्या', crops: ['वांगी', 'कोबी', 'पत्ता कोबी', 'टोमॅटो', 'डिंक गवार', 'कांदा', 'बटाटा', 'गवार', 'कारले', 'चवळी', 'तिखट मिरची', 'भेंडी', 'दुधी भोपळा', 'कॉलीफ्लॉवर', 'आले', 'घेवडा', 'दोडका', 'हळद', 'डांगर भोपळा', 'ढोबळी मिरची', 'शेवगा', 'काकडी', 'गाजर', 'मुळा', 'पालक', 'ब्रॉकली', 'घोसाळे', 'टिंडा', 'ढेमसे', 'लसूण', 'कोथिंबीर', 'मेथी पाला', 'कांद्याची पात'], icon: '🥬', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
              { name: 'डाळी', crops: ['उडीद', 'चणा', 'मुग', 'तूर', 'मसूर', 'राजमा'], icon: '🫘', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
              { name: 'तेलबिया', crops: ['तीळ', 'एरंड', 'भुईमूग', 'मोहरी', 'सोयाबीन', 'सूर्यफूल', 'कुसुम'], icon: '🌻', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
              { name: 'तृणधान्ये', crops: ['बाजरी', 'मका', 'भात', 'गहू', 'गोड मका', 'ज्वार', 'नाचणी', 'वरई', 'जव'], icon: '🌾', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
              { name: 'फळे', crops: ['कलिंगड', 'पपई', 'आंबा', 'डाळिंब', 'खरबूज', 'केळे', 'द्राक्षे', 'मोसंबी', 'संत्रा', 'लिंबू', 'काजू', 'अंजीर', 'पेरू', 'सीताफळ', 'बीट', 'रामफळ', 'जांभूळ', 'बोर', 'कोकम', 'फणस', 'नारळ', 'सुपारी', 'ड्रॅगन फ्रूट', 'अवोकाडो'], icon: '🍎', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
              { name: 'मसाले', crops: ['जिरे', 'बडीशेप', 'मेथी', 'धणे', 'काळी मिरी', 'तमालपत्र', 'लवंग'], icon: '🌶️', image: 'https://customer-assets.emergentagent.com/job_greetings-1483/artifacts/d9ze41ct_Screenshot_20251206-163005.png' },
                ...
                <div key={category.name} className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  <div className="flex items-center space-x-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{categoryBlogs.length} पोस्ट उपलब्ध</p>
                    </div>
                  </div>

                  {/* Crops Dropdown Filter */}
                  <div className="mb-6">
                    <div className="flex flex-wrap gap-2">
                      {category.crops.map(crop => {
                        const cropBlogsCount = blogs.filter(b => b.selectedCrop === crop).length;
                        if (cropBlogsCount === 0) return null;
                        
                        return (
                          <button
                            key={crop}
                            onClick={() => {
                              document.getElementById(`crop-${crop}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                            }}
                            className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 rounded-lg text-sm font-semibold transition"
                          >
                            {crop} ({cropBlogsCount})
                          </button>
                        );
                  ...
                  {category.crops.map(crop => {
                    ...
                    if (cropBlogs.length === 0) return null;

                    return (
                      <div key={crop} id={`crop-${crop}`} className="mb-8 scroll-mt-20">
                        <h4 className="text-xl font-bold text-gray-800 mb-4 flex items-center space-x-2">
                          <span>→</span>
                          <span>{crop}</span>
                          <span className="text-sm text-gray-500">({cropBlogs.length} पोस्ट)</span>
                        </h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {cropBlogs.map(blog => {
                            const layoutAspects = {
                            ...
                            return (
                              <div key={blog.id} className="bg-gray-50 rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col">
                                <div className="relative w-full bg-gray-100" style={{ paddingBottom: `${(1 / aspect) * 100}%` }}>
                                  <img 
                                    src={applyCloudinaryOptimization(blog.image) || 'https://via.placeholder.com/400x300?text=Blog+Image'} 
                                    alt={`${crop} Blog`}
